...
{/* --- FLOATING BUTTONS (WhatsApp + ScrollToTop) --- */}
      <div className="fixed bottom-6 right-6 lg:bottom-8 lg:right-8 flex flex-col gap-4 z-50">
        ...
        <motion.a
          ...
          // MOBILE : w-16 h-16 (64px) -> Taille idéale (Entre 48px et 80px)
          // DESKTOP : w-12 h-12 (48px)
          className="w-16 h-16 lg:w-12 lg:h-12 bg-[#25D366] text-white flex items-center justify-center shadow-xl transition-all duration-300 rounded hover:scale-110"
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
          whileHover={{ y: -5 }}
        >
          {/* Icône WhatsApp : w-10 (40px) sur mobile */}
          <svg viewBox="0 0 24 24" className="w-10 h-10 lg:w-7 lg:h-7 fill-current" xmlns="http://www.w3.org/2000/svg">
        ...
        <motion.button
          onClick={scrollToTop}
          // MOBILE : w-16 h-16
          className="w-16 h-16 lg:w-12 lg:h-12 bg-primary text-primary-foreground flex items-center justify-center shadow-xl transition-colors duration-300 rounded hover:bg-foreground hover:text-background"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.95 }}
        >
          {/* Flèche : w-8 h-8 sur mobile */}
          <ArrowUp strokeWidth={3} className="w-8 h-8 lg:w-5 lg:h-5" />
        </motion.button>
