    ...
    <footer className="bg-background border-t border-border relative">
      ...
      <div className="container mx-auto px-6 lg:px-12 py-16 lg:py-24">
        <div className="grid md:grid-cols-1 lg:grid-cols-5 gap-12">
          ...
          <div className='w-2/3 lg:w-full mx-auto text-center lg:text-left'>
            ...
            {status === "success" ? (
              <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-6 lg:p-4 flex flex-col items-center justify-center gap-3 text-emerald-400 animate-in fade-in zoom-in">
                <CheckCircle className="w-10 h-10 lg:w-6 lg:h-6" />
                <p className="font-medium text-xl lg:text-sm">Merci ! Vous êtes inscrit.</p>
              </div>
            ) : (
              <form className="flex flex-col gap-5 lg:gap-3" onSubmit={handleSubscribe}>
                <div className="relative">
                    ...
                    className="w-full px-6 py-5 lg:px-4 lg:py-3 bg-card border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary transition-colors duration-300 text-xl lg:text-base"
                    required
                    />
                </div>

                {status === "error" && (
                    <div className="text-red-400 text-lg lg:text-xs flex items-center justify-center lg:justify-start gap-2">
                        <AlertCircle className="w-5 h-5 lg:w-3 lg:h-3" />
                        {errorMessage}
                    </div>
                )}
                ...
                <button
                  ...
                  className="w-full px-6 py-5 lg:px-4 lg:py-3 bg-gradient-gold text-primary-foreground font-medium hover:bg-foreground hover:text-background transition-colors duration-300 text-xl lg:text-base flex items-center justify-center gap-2 disabled:opacity-70"
                >
                  {status === "loading" ? (
                    ...
                    <>
                        S'abonner
                        {/* <Send className="w-5 h-5 lg:w-4 lg:h-4 ml-1" /> */}
                    </>
                  )}
                </button>
              </form>
      ...
      <div className="fixed bottom-6 right-6 lg:bottom-8 lg:right-8 flex flex-col gap-6 lg:gap-4 z-50">
        ...
        <motion.a
          ...
          className="w-24 h-24 lg:w-12 lg:h-12 bg-[#25D366] text-white flex items-center justify-center shadow-xl transition-all duration-300 rounded hover:scale-110"
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
          whileHover={{ y: -5 }}
        >
          <svg viewBox="0 0 24 24" className="w-16 h-16 lg:w-7 lg:h-7 fill-current" xmlns="http://www.w3.org/2000/svg">
        ...
        <motion.button
          onClick={scrollToTop}
          className="w-24 h-24 lg:w-12 lg:h-12 bg-gradient-gold text-primary-foreground flex items-center justify-center shadow-xl transition-colors duration-300 rounded hover:bg-foreground hover:text-background"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.95 }}
        >
          <ArrowUp strokeWidth={3} className="w-14 h-14 lg:w-5 lg:h-5" />
        </motion.button>
